...
import { TimerStates } from "../utility/_ts_only/types";
...
export class Timer {
	...
	private _state = new StateMachine<[TimerStates]>("Ready");
	...
	public GetState(): TimerStates | undefined {
	...
	public Start(): void {
		...
		this._run_thread = coroutine.create(() => {
			this.janitor.Add(
				(this._run_connection = RunService.Heartbeat.Connect((dt) => {
					if (this._state.GetState() !== "Running") return;
