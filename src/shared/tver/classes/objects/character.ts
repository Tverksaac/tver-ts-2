...
export class Character {
	...
	private replication_done: boolean = is_client_context();
	...
	constructor(from_instance: Instance) {
		...
		const _properties = [new ConnectedProperty<Humanoid, "AutoRotate">(this.humanoid, "AutoRotate", true)];
	...
	private _compound_effect_only_apply_effect(applied_effect: AppliedCompoundEffect) {
		...
		applied_effect.janitor.Add(
			applied_effect.state.StateChanged.Connect(() => {
	...
	private _start_listen_to_effect_changes(): void {
		const listen_to: Signal[] = [this.EffectApplied, this.EffectRemoved, this._effect_changed];
	...
	private _calculate_stat_effects(): Map<string, { Affects: string; Raw: number; Modifier: number }> {
		...
		this._stat_effects.forEach((effect) => {
			...
			if (effect_type === "Modifier") {
				...
				log.w(
					effect +
						" have wrong effect type property! /n Should be 'Raw' or 'Modifier' but have value of: " +
						effect.EffectType,
				);
				return;
			}
		...
		calculated.forEach((prop) => {
			...
			if (prop_to_affect) {
				...
				if (typeOf(prop.Strength) === typeOf(prop_to_affect.Get())) {
					...
					log.e(
						prop +
							" has wrong Strength value!\nCannot assign " +
							typeOf(prop.Strength) +
							" to " +
							typeOf(prop_to_affect.Get()),
					);
				}
			} else {
				...
				if (prop_to_affect) {
					...
					log.w(
						prop + " Cant affect any of properties because no property exists with name: " + prop.Affects,
					);
					return;
				}
			}
	...
	private _connect_server_atom_updating(): void {
		const signals = [this.EffectApplied, this.EffectRemoved, this.SkillAdded, this.SkillRemoved];
	...
	private init(): boolean {
		...
		while (!this.replication_done && !is_client_context()) {
			task.wait();
		} // yield until replciation is done
